...
if grep -q "autostartfix.sh" /opt/retropie/configs/all/emulationstation/gamelists/retropie/gamelist.xml ; then
        ...
        sudo sed -i 's|</gameList>|\t<game>\n\t\t<path>./Update-Addons/Fixes/autostartfix.sh</path>\n\t\t<name>Fix autostart.sh</name>\n\t\t<desc>ENG: Menu to modify the retropie autostart.sh script to have the option to select kelboy launcher at boot and have joystick.py also work with the ES and Kodi options. It could be neccessary to apply this fix after an update.\nESP: Menu que modifica el script de retropie autostart.sh para tener la opción de seleccionar el launcher de la kelboy para que inicie en el arranque y tambien hacer que joystick.py también funcione con las opciones ES y Kodi (con esto conseguimos por ejemplo tener icono de bateria arrancando en ES sin launcher). Podría ser necesario aplicar esta corrección después de una actualización.</desc>\n\t\t<image></image>\n\t\t<playcount>0</playcount>\n\t\t<lastplayed>20180514T205700</lastplayed>\n\t</game>\n</gameList>|' /opt/retropie/configs/all/emulationstation/gamelists/retropie/gamelist.xml
fi
